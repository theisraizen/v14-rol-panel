...
client.on(Events.ClientReady, async() => {
    ...
    client.user.setActivity(`${info.roledurum}`, { type: ActivityType.Custom });
...
if(memberRoles === 0) {
   ...
   interaction.reply({ content: `Başarıyla <@&${info.kayıtsız}> rolünü verdim ve ismini \`${info.tag} İsim | Yaş\` olarak ayarladım <#${info.registerchat}> kanalına yazabilirsin ^^`, ephemeral: true })
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        const takımroller = [info.takım.besik, info.takım.fener, info.takım.galat, info.takım.trabzon]
